...
import myhdl
from myhdl import intbv, EnumItemType, EnumType, modbv, delay, posedge, negedge, concat, now, bin
...
class _ToVerilogConvertor(object):
    ...
    def __call__(self, func, *args, **kwargs):
        ...
        vfilename = name + (".sv" if self.standard >= 'SV2005' else ".v")
...
def _writeSigDecls(f, intf, siglist, memlist):
    ...
    for s in siglist:
        ...
        if s._driven:
            ...
            constwires.append(s)
            print("%s %s%s;" % ('wire' if toVerilog.standard < 'SV20005' else 'wire ', r, s._name), file=f)
    print(file=f)
    for m in memlist:
        if not m._used:
            continue
